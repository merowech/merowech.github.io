    ...
    rawFile.onreadystatechange = function() {
        if (rawFile.readyState === 4) {
            if (rawFile.status === 200 || rawFile.status == 0) {
                ...
                allText = scaleToWidth(allText, line_length, $("body").css("font"));
...
var scaleToWidth = function(text, line_length, font) {
    var splits = text.split(/^\n/gm);

    var container_helper = $('<span></span>').css({
        display: 'none'
    }).appendTo($('body'));

    var canvas = scaleToWidth.canvas || (scaleToWidth.canvas = document.createElement("canvas"));
